...
export default function NewBlogPost() {
    ...
    // let preview_src = useSearchParams().get("preview") ?? "";
    let [keywords,setKeywords] = useState<string>("");
    let [preview,setPreview] = useState<string>("");
    let debouncedPreview = useDebounce(preview,1000);
    // let [previewSrc,setPreviewSrc] = useState<any>();
    // useEffect(()=>{
    //     serialize(debouncedPreview || "?").then((v)=>{
    //         setPreviewSrc(v);
    //     });
    // },[debouncedPreview])
    ...
    return <div className="flex mx-auto p-20 flex-col">
        <div className="w-fit mx-auto bg-neutral-800 border-2 shadow-xl border-neutral-600 rounded-xl p-3 flex flex-col" >
            ...
            <form action={SubmitData} className="flex flex-col w-[42rem] px-4">
                ...
                <textarea value={preview} onChange={(v)=>setPreview(v.target.value)}  required className="text-white border border-neutral-500 appearance-none outline-none focus:ring-1 hover:ring-1 hover:ring-fuchsia-600 focus:ring-fuchsia-700 rounded-lg bg-neutral-700 px-3 py-2" id="content" name="content" />
                ...
                <input value={keywords} onChange={(v)=>setKeywords(v.target.value)} required className="text-white border border-neutral-500 appearance-none outline-none focus:ring-1 hover:ring-1 hover:ring-fuchsia-600 focus:ring-fuchsia-700 rounded-lg bg-neutral-700 px-3 py-2" id="kew" name="kew"  type="text" />
